...
resolutions.
  ...
  B16       Booking window    `end_time` can    Missing explicit     Add `if end <= start`
          validation        be before or      validation before    check after datetime
          missing           equal to          duration             parsing; return `400`
                            `start_time`.     calculation.         `INVALID_BOOKING_WINDOW`
                            This allows                             with message
                            invalid booking                         `end_time must be after
                            payloads and                            start_time`.
                            unclear duration
                            errors.
  ------------------------------------------------------------------------
...
**B24 — Booking detail readable by any member in the org.** Rule 10 (members may read
...
**B25 — Creating a room serves a stale usage report.** Rule 12 (report includes every
